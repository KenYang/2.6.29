...
static int spu_write(struct if_spi_card *card, u16 reg, const u8 *buf, int len)
{
	...
	memcpy(card->iodata, &reg_out, sizeof(u16));
    	memcpy(card->iodata + sizeof(u16), buf, len);
	...
	x.tx_buf = (u16 *)card->iodata;
	x.len = SET_TX_RX_LEN((len>>1)+1,0);
...
static int spu_read(struct if_spi_card *card, u16 reg, u8 *buf, int len)
{
	...
	x.tx_buf = &reg_out;
	x.rx_buf = buf;
	x.len =SET_TX_RX_LEN(sizeof(reg_out)>>1, len>>1);
	spi_message_add_tail(&x, &m);
...
static int __devinit if_spi_probe(struct spi_device *spi)
{
	...
	card->iodata = kzalloc(SZ_4K, GFP_KERNEL);
	if (!card->iodata) {
		err = -ENOMEM;
		goto out;
	}
